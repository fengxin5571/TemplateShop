<template>
  <div class="item_detail">
    ...
    <van-cell-group class="item_cell_group" v-if="goods">
      <van-cell class="item_info">
        <div>
          <span class="item_price">{{ goods.info.retail_price*100 | yuan }}</span>
          <span class="item_market_price">{{goods.info.counter_price*100 | yuan}}</span>
        </div>
  ...
  <div class="item_cell_group">
    <detail-help :isuees="issues"></detail-help>
  </div>
    <div class="item_desc">
  ...
  methods: {
    ...
    initData() {
      goodsDetail({ id: this.itemId }).then(res => {
        ...
        this.skuAdapter();
        this.issues=this.goods.issue.list;
      });
    ...
    skuAdapter() {
      ...
      const skuInfo = {
        price: parseInt(this.goods.info.retail_price), // 未选择规格时的价格
      ...
      this.skuGoods = {
        ...
        picture: this.goods.info.pic_url
      };
    },
    setSkuList() {
      var sku_list = [];
